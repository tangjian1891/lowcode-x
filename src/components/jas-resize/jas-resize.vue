<template>
  <div ref="jasTwoResizeRef" :draggable="false" :class="['jas-resize', `jas-resize--${direction}`, { events_none: mouse.isMoving }]">
    <div
      class="resize-panel resize-panel-first"
      :style="direction === 'horizontal' ? { width: firstPanelSize + 'px' } : { height: firstPanelSize + 'px' }"
    >
      <slot name="p1"></slot>
    </div>
    <jas-resize-bar
...
const mouse = reactive({
  ...
  mousemove(e) {
    ...
    // 计算移动距离并更新尺寸
    const delta = direction.value === "horizontal" ? e.clientX - mouse.startPosition : e.clientY - mouse.startPosition;
